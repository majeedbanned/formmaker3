...
export default function SkyroomMeetingPage() {
  const params = useParams();
  const className = params.className;
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    // Here you would typically connect to the Skyroom API
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-2xl w-full">
        <h1 className="text-2xl font-bold text-center mb-6">کلاس آنلاین Skyroom</h1>
        <h2 className="text-xl text-center mb-8">{className}</h2>
        
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
          <p className="text-blue-700">
            این یک صفحه نمونه است. در نسخه نهایی، کاربر به صورت خودکار به جلسه Skyroom متصل خواهد شد.
          </p>
        </div>
