...
export async function POST(request: NextRequest) {
  try {
    ...
    const { classCode, courseCode, timeSlot, date: requestedDate } = body;
    ...
    const persianDate = formatJalaliDate(workingDate);
    const persianMonthName = getPersianMonthName(jMonth);
    ...
    try {
      ...
      if (students.length === 0) {
        await client.close();
        return NextResponse.json(
          { success: false, message: 'لیست دانش‌آموزان خالی است' },
          { status: 400 }
        );
      }
      ...
      const result = await db.collection('classsheet').bulkWrite(bulkOps);
      ...
      return NextResponse.json({
        success: true,
        message: 'حضور همگانی با موفقیت ثبت شد',
    ...
    return NextResponse.json(
      { success: false, message: 'خطا در ثبت حضور همگانی' },
      { status: 500 }
    );
  }
}
