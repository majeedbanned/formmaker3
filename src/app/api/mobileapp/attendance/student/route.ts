...
export async function GET(request: NextRequest) {
  try {
    ...
    if (decoded.role !== 'student' && decoded.userType !== 'student') {
      return NextResponse.json(
        { success: false, message: 'دسترسی غیرمجاز - فقط دانش‌آموزان می‌توانند سوابق حضور و غیاب خود را مشاهده کنند' },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      const transformedRecords: StudentAttendanceRecord[] = attendanceRecords.map(record => {
      ...
      const presentCount = transformedRecords.filter(r => r.presenceStatus === 'present').length;
      const absentCount = transformedRecords.filter(r => r.presenceStatus === 'absent').length;
      const lateCount = transformedRecords.filter(r => r.presenceStatus === 'late').length;
