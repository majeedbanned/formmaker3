...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Simple Persian date format (you can enhance this with proper Persian date conversion)
      const readPersianDate = now.toLocaleDateString('fa-IR');

      // Update message as read
      const result = await db.collection('messagelist').updateOne(
        { 
          _id: new ObjectId(messageId),
          'data.receivercode': decoded.username // Ensure user can only mark their own messages as read
        },
        {
          $set: {
            'data.isRead': true,
      ...
      if (result.matchedCount === 0) {
        return NextResponse.json(
          { success: false, message: 'پیام یافت نشد یا شما اجازه دسترسی به این پیام را ندارید' },
          { status: 404 }
        );
      }
