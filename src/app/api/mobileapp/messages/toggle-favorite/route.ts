...
export async function POST(request: NextRequest) {
  try {
    ...
    const { messageId, isFavorite } = body;

    if (!messageId || typeof isFavorite !== 'boolean') {
      return NextResponse.json(
        { success: false, message: 'شناسه پیام و وضعیت نشان الزامی است' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const result = await db.collection('messagelist').updateOne(
        { 
          ...
          $or: [
            { 'data.receivercode': decoded.username }, // User can favorite their received messages
            { 'data.sendercode': decoded.username }    // User can favorite their sent messages
          ]
        },
        {
          $set: {
      ...
      return NextResponse.json({
        success: true,
        message: isFavorite ? 'پیام به نشان‌ها اضافه شد' : 'پیام از نشان‌ها حذف شد',
        data: {
          messageId,
          isFavorite
        }
