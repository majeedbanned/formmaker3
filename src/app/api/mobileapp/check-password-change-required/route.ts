...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only check for teachers and schools
    if (decoded.userType !== 'teacher' && decoded.userType !== 'school') {
      return NextResponse.json({
        success: true,
        requiresPasswordChange: false,
        message: 'این بررسی فقط برای معلمان و مدیران مدرسه است'
      }, { headers: corsHeaders });
    }
    ...
    try {
      ...
      if (decoded.userType === 'teacher') {
        ...
        user = await db.collection('schools').findOne({
          $or: [
            ...
            { 'data.schoolCode': decoded.username }
          ],
          'data.schoolCode': decoded.schoolCode,
          'data.isActive': true,
          'data.domain': decoded.domain
        });

        if (user) {
          username = user.data.username || user.data.schoolCode || '';
          password = user.data.password || '';
        }
      }
      ...
      const requiresPasswordChange = username === password && username !== '' && password !== '';
    ...
    console.error('Check password change required API error:', error);
    return NextResponse.json(
