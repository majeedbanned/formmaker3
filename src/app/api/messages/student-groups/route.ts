...
export async function POST(request: Request) {
  try {
    // Get request body
    const { groupCodes, schoolCode } = await request.json();
    ...
    const students = await StudentModel.find({
      ...
      $or: [
        // Match when groups is an array of objects with value property
        {'data.groups': {$elemMatch: {value: {$in: groupCodes}}}},
        // Alternative query if structure is different
        {'data.groups.value': {$in: groupCodes}}
      ]
    }).exec();
    ...
    console.error("Error fetching students in groups:", error);
    if (error instanceof Error) {
      console.error("Error details:", error.message, error.stack);
    }
    return NextResponse.json(
      { error: "Failed to fetch students in groups" },
      { status: 500 }
    );
  }
} 