...
export async function GET(
  ...
  try {
    ...
    const scores = grades.map(g => g.score).sort((a, b) => b - a);
    const average = scores.length > 0 ? scores.reduce((sum, score) => sum + score, 0) / scores.length : 0;

    // Calculate rankings and percentiles
    const studentsWithAnalytics = grades.map(grade => {
    ...
    let currentRank = 1;
    studentsWithAnalytics.forEach((student, index) => {
      if (index > 0 && student.score < studentsWithAnalytics[index - 1].score) {
        currentRank = index + 1;
      }
