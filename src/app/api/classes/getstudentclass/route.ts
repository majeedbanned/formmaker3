...
export async function POST(req: NextRequest) {
  try {
    ...
    const { schoolCode, studentCode } = await req.json();
    ...
    // For students, verify they are only accessing their own class
    if (
      currentUser.role === "student" &&
      currentUser.username !== studentCode
    ) {
      return NextResponse.json(
        { error: "Students can only view their own class" },
        { status: 403 }
      );
    }
